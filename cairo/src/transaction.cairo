...
from buffer import Reader, read_uint32, read_uint64, read_varint, read_hash, read_bytes, UINT32_SIZE
...
func read_transaction{reader:Reader, range_check_ptr}() -> (transaction: Transaction):
	alloc_locals
...
func read_inputs{reader:Reader, range_check_ptr}(inputs_len) -> (inputs: TxInput*):
	alloc_locals
...
func _read_inputs_loop{reader:Reader, range_check_ptr}(inputs: TxInput*, inputs_len):
	if inputs_len == 0:
		return ()
	end
	let (input) = read_input()
	assert [inputs] = input
	_read_inputs_loop(inputs + TxInput.SIZE, inputs_len - 1)
	return ()
end

# Read a transaction input from a buffer
func read_input{reader:Reader, range_check_ptr}() -> (input: TxInput):
	alloc_locals
	let (txid)				= read_hash()
...
func read_outputs{reader:Reader, range_check_ptr}(outputs_len) -> (outputs: TxOutput*):
	alloc_locals
...
func _read_outputs_loop{reader:Reader, range_check_ptr}(outputs: TxOutput*, outputs_len):
	if outputs_len == 0:
		return ()
	end
	let (output) = read_output()
	assert [outputs] = output
	_read_outputs_loop(outputs + TxOutput.SIZE, outputs_len - 1)
	return ()
end

# Read an output from a buffer
func read_output{reader:Reader, range_check_ptr}() -> (output: TxOutput):
	alloc_locals
...
func read_transaction_validation_context{reader:Reader, range_check_ptr}(
