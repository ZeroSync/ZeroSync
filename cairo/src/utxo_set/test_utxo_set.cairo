...
func test_utxo_set{range_check_ptr, pedersen_ptr: HashBuiltin*}():
	...
	%}
	utxo_set_insert{hash_ptr=pedersen_ptr, utreexo_roots=utreexo_roots}(txid, vout, amount, script_pub_key, script_pub_key_size)

	let (out_amount, out_script_pub_key, out_script_pub_key_len) = utxo_set_extract{hash_ptr=pedersen_ptr, utreexo_roots=utreexo_roots}(txid, vout)
